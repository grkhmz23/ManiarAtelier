...
const WomenPageHero = ({ onBack, t }: { onBack: () => void; t: any }) => {
  ...
  return (
    <section className="relative w-full h-[70vh] min-h-[550px] bg-[#FDFBF7] flex items-center justify-center px-6 md:px-12 border-b border-[#0B1026]/10 overflow-hidden">
      ...
      <button
        onClick={onBack}
        className={`absolute top-8 ${isRTL ? 'right-6 md:right-12' : 'left-6 md:left-12'} z-20 flex items-center gap-2 text-xs uppercase tracking-widest text-[#0B1026]/60 hover:text-[#D6AC54] transition-colors`}
      >
        <ArrowLeft size={14} />
